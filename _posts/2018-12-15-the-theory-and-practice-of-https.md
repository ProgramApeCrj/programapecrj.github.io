...
其实这是因为我以为一套对称加密解决所有问题，这是理解的误区。上面两个问题对应两种应用场景： RSA 签名方案 和 RSA 加密方案。
...
这种情况下：只能保证内容不被串改，不保证内容不被别人知道。比如你发的是一条公告新闻，保证每个人看得到，但是别人伪造不了。
...
浏览器后面每次发起请求都带上这段内容（token），服务端要认证，其实就是用秘钥对前面的用户信息做一次 HMAC，然后判断是否和后面的内容一致，就可以知道前面的用户信息有没有被修改过，确保无误则承认该用户，用于后面其他操作。
...
但是，对于请求方来说，它怎么能确定它所得到的公钥一定是从目标主机那里发布的，而没有中间人篡改过呢？一旦这个公钥不可靠（比如黑客伪造的），那请求方发的内容就很有可能被黑客劫持并解密。

如果在线发送公密钥，肯定要通过互联网传输，上面我们说过网络是不安全的，那如何保证约定的公秘钥不泄露? 再来一层公秘钥加密？那第二层的公秘钥如何传输?这样就陷入死循环中了。

最好的办法就是把公钥直接到你家里给你手动输入到电脑。哈哈，可能吗？答案是可能，这就是 https 了。只不过你保存的不是某个网站的公钥，而是某个权威机构的公钥。

https 的权威机构（CA）和电脑操作系统或浏览器厂商合作，内置了自己的公钥（这就是我说的到你家给你输入），从而保障不可篡改。

这就有了如下的 https 请求过程:

首先，客户端向服务器发出加密请求。 

服务器用自己的私钥加密网页（或信息）以后，连同本身的数字证书，一起发送给客户端。 

客户端（浏览器）的 ”证书管理器” 中有 ”受信任的根证书颁发机构” 列表。客户端会根据这张列表，查看解开数字证书的公钥是否在列表之内。

然后用 CA 权威机构的公钥解开证书，证书记录的内容就是网址和其他信息（比如域名），如果发现没有问题，就可以证明这张证书的确是 CA 机构发布的，浏览的这个网站也没有问题，那就取出证书里网站真正的公钥，开始加密传输。

一旦解不开或者发现信息对应不上，浏览器就会提示你，你的信息要小心泄露了 。
...
1. https 只是保证网站数据传输安全（加密），但是如果这个网站一开始就是打算骗你钱的，那数据传输安全不安全也没意义了哈哈。
