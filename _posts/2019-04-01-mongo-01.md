...
假设我取了一个 num 字段进行分片，这个 num 字段是数字，一开始就只有一个分片，(minNum，maxNum)。

这时候随着数据增加，超过了阈值 chunkSize ，而分片服务器足够多的情况下，mongo 会取一个中间值，然后把数据转移切割。比如这时候大于 100 和 小于 100 的都差不多，那 mongo 就会把分片变成 (minNum,100] 和 (100, maxNum)
